...
function MonthManager({ materials, setMaterials }: { materials: Material[]; setMaterials: (materials: Material[]) => void }) {
  ...
  const deleteMonthFromAllMaterials = async () => {
    ...
    try {
      const allMaterials = await getMaterials()
      const updatedMaterials = allMaterials.map((material) => {
        const updatedPrices = material.Prices.filter(([date]) => date !== monthToDelete)
        return {
          ...material,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-4">
        ...
        <Button variant="outline" className="text-destructive" onClick={deleteMonthFromAllMaterials}>
